...
export default async function AdminOnboardingPage() {
  ...
  const recent = clinics.map((clinic, index) => ({ ...clinic, owner: ["Marta Silva", "Daniel Roca", "Irene Vidal"][index] ?? "Clinic owner", progress: clinic.status === "active" ? 100 : 62 }));
        ...
        <div className="divide-y divide-[#eadfd8]">
          {recent.map((clinic) => (
            <Link key={clinic.id} href={`/admin/clinics/${clinic.id}`} className="grid gap-3 px-5 py-4 transition hover:bg-[#fffaf6] md:grid-cols-[1fr_160px_180px_24px] md:items-center">
              <span><span className="block font-semibold">{clinic.name}</span><span className="mt-1 block text-xs text-[#806d63]">{clinic.owner} · {clinic.city}</span></span>
